...
def read_table(file_root, filename, study):
    patient_file_name = os.path.join(file_root, filename)
    if "xlsx" in patient_file_name:
        patient_data = pd.read_excel(patient_file_name)
    else:
        try:
            patient_data = pd.read_csv(patient_file_name)
        except UnicodeDecodeError:
            patient_data = pd.read_csv(patient_file_name, encoding='cp949')

    # Renaming columns if necessary
    if 'PaymentID' in patient_data.columns:
        patient_data.rename(columns={'PaymentID': 'Payment_Number'}, inplace=True)
    elif 'Payment_Number' not in patient_data.columns:
        patient_data['Payment_Number'] = patient_data['ID']

    # Converting Payment_Number to string
    patient_data['Payment_Number'] = patient_data['Payment_Number'].astype(str)
    
    # Ensure certain columns do not exist
    assert not set(["Op_Code", "Op_Year", "Op_Date", "Op_Dur", "Dur_Adm", "Op_Name", "Op_Dx"]).issubset(patient_data.columns), \
        "Columns Op_Code, Op_Year, Op_Dur, Dur_Adm should not be present in the data."
    
    # Apply the mapping to the 'Op_Dep' column
    opdep_mapfunc = {
        "OS": "5", # 정형외과
        "NS": "4", # 신경외과
        "UR": "3", # 비뇨기과
    ...
    patient_data["Op_Dep"] = patient_data["Op_Dep"].apply(lambda x: opdep_mapfunc[x] if x in opdep_mapfunc else x)
    patient_data['Op_Dep'] = patient_data['Op_Dep'].astype(str)

    # Filtering data based on the study
    # if "-" in study: #BRMH|KNUH|CAUH
    #     studies = study.split('-')
    #     patient_data = patient_data[patient_data['Study'].isin(studies)]
    # else:
    #     patient_data = patient_data[patient_data['Study'] == study]

    if "-" in study:
        studies = study.split('-')
        patient_data = patient_data[patient_data['Study'].isin(studies)]

        # Separating data by studies
        cauh_data = patient_data[patient_data['Study'] == 'CAUH']
        brmh_data = patient_data[patient_data['Study'] == 'BRMH']
        knuh_data = patient_data[patient_data['Study'] == 'KNUH']

        cauh_data['A_HT'], cauh_data['A_WT'] = cauh_data['A_WT'].copy(), cauh_data['A_HT'].copy()

        # Concatenating modified CAUH data with other study data
        patient_data = pd.concat([cauh_data, brmh_data, knuh_data])
        
    else:
        patient_data = patient_data[patient_data['Study'] == study]
        if study == 'CAUH':
            patient_data['A_HT'], patient_data['A_WT'] = patient_data['A_WT'].copy(), patient_data['A_HT'].copy()

    # Dropping columns with all missing values and removing duplicate rows
...
class ProcessTable():
    ...
    def remove_useless_column(self):
        """
        Removes specific columns from patient_data that are deemed unnecessary for further analysis.
        This includes IDs, automatically generated columns, and domain-specific columns that are either redundant
        or can be recalculated. Changes in columns as of 2023-06-16 are noted.
        """
        # List all columns to drop, including both general and domain-specific columns.
        columns_to_drop = [
            'Study', 'id_for_ordering', 'ID', 'Unnamed: 0', '사망원인1', '사망원인2',  # General columns
            'A_Birth', 'First_CRRT', 'Last_CRRT', 'ICU_ADM', 'ICU_detailed', 'ICU_DISCHARGE', # Filter for Hepatectomy dataset (old version)
            'F_Cr_Max', 'B_CAPD_ESRD',  # Specific domain knowledge columns
            'A_BMI', 'B_eGFR_EPI_near', 'B_eGFR_EPI_stage_near',  # Columns recalculable with logic
            'A_SMK1', 'AN_DM', 'AN_HTN', 'B_DM', 'B_HTN', 'O_AKI_criteria1', 'O_AKI_criteria2', 
            'O_HD_7', 'O_HD_90', 'O_CRRT_7', 'O_CRRT_90',  # Higher group definition process columns
            'AKI_Cr2_Days_after_Op_start', 'AKI_first_Stage', 'AKI_Max_Stage', 
            'AKI_Cr2_Cr_diff', 'AKI_Cr2_Value', 'AKI_Max_Value', 'AKI_first_Value'  # Additional specific columns
        ]

        # Drop all specified columns, ignoring errors for non-existent columns.
        self.patient_data.drop(columns=columns_to_drop, axis=1, inplace=True, errors='ignore')

        # Identify and remove all date and time-related columns dynamically
        date_time_columns = self.patient_data.filter(regex=r'(?i)(date|time)').columns
        self.patient_data.drop(columns=date_time_columns, axis=1, inplace=True, errors='ignore')
    ...
    def merge_columns(self, obj_patient_data, int_patient_data, float_patient_data):
        ...
        if self.args.boxcox_transformation:
            print("Box-cox transformation...")
            results = boxcox_transform(df, numerical_columns, self.args)
            df = results.copy()
        ...
        info_dict = {
            'cat_vocab_size' : vocab + 3,  # cat_null_id + pad + mask
...
def boxcox_transform(df, numeric_cols, args):
    
    if ("-" in args.study) and (args.study == "BRMH-KNUH-CAUH"):
    ...
    for col in numeric_cols:
        ...
        if p < 0.05:
            ...
            transformed_data, lambda_val = boxcox(non_na_data)
            if abs(lambda_val) > 5:
                results[col] = non_na_data
            else:
    ...
    lambda_df = pd.DataFrame(list(lambdas.items()), columns=['Column', 'Lambda'])
    lambda_df.to_csv(os.path.join(args.output_path, args.norm, 'lambda_values.csv'), index=False)
    return results
...
def merge_text_table(table_df, unnorm_table_df, file_root, sub_root, cat_null_ids, num_null_ids, cat_col_num, num_col_num, text_df=None):
    ...
    if text_df is not None:
        ...
        text_tokens = np.ones((df.shape[0], text_df.shape[1]-1), dtype=np.int)
    else:
        df = table_df.copy()
    ...
    #token_type_ids
    table_tokens = np.zeros((df.shape[0], table_df.shape[1]-1), dtype=np.int16)
...
def fold_split(total_data, target_fold_root):
    ...
    for seed in seed_list:
        ...
        print(seed, ind_list[:train_num][:10])
...
def get_parser():
    ...
    parser.add_argument('--input_path', type=str, default='/data/source_data')
    parser.add_argument('--preprocessed_op_filename', type=str, default='dataset_op_report.csv')
    parser.add_argument('--patient_data_filename', type=str, default='tabular_data.xlsx')
    ...
    parser.add_argument('--study', type=str, default="AMC", choices=['AMC', 'SNUH', 'SNUBH', 'BRMH', 'KNUH', 'CAUH', 'KUMC'])
    parser.add_argument('--boxcox_transformation', action='store_true') 
    return parser
...
if __name__ == '__main__':
    ...
    patient_data = read_table(args.input_path, args.patient_data_filename, args.study)
    text_df = None
    ...
    # Save as JSON (replacing pickle)
    import json
    configs_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), 'configs')
    os.makedirs(configs_dir, exist_ok=True)
    
    # Convert numpy types to Python types for JSON serialization
    # Note: cat_vocab_size is not stored - it will be calculated as cat_null_ids + 3
    json_config = {
        'cat_col_num': int(info_dict['cat_col_num']),
        'num_col_num': int(info_dict['num_col_num']),
        'cat_null_ids': int(info_dict['cat_null_ids']),
        'num_null_ids': int(info_dict['num_null_ids']),
        'column_order': info_dict['column_order'],
        'cat_columns_numclass_list': [int(x) for x in info_dict['cat_columns_numclass_list']],
        'text_num': int(ntext) if ntext is not None else None
    }
    
    # Save to configs directory for easy access
    config_path = os.path.join(configs_dir, f'{args.study}.json')
    with open(config_path, 'w') as f:
        json.dump(json_config, f, indent=2)
    
    # Also save to data directory for backward compatibility with loaders
    data_config_path = os.path.join(args.output_path, args.norm, 'config.json')
    with open(data_config_path, 'w') as f:
        json.dump(json_config, f, indent=2)
    
    print(f'>>> Saving config to {config_path} is done ..')
    print(f'>>> Saving config to {data_config_path} is done ..')
